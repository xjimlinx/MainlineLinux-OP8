...
ColumnLayout {
    ...
    // ConfigView can expose an empty currentWallpaper while its model is still
    // being populated. Keep a usable image-plugin fallback for that interval.
    property string currentWallpaper: configDialog.currentWallpaper || "org.kde.image"
        ...
        FormCard.FormCard {
            ...
            FormCard.FormComboBoxDelegate {
                ...
                onCurrentIndexChanged: {
                    if (currentIndex < 0 || currentIndex >= configDialog.wallpaperConfigModel.count) {
                        return;
                    }
                    var model = configDialog.wallpaperConfigModel.get(currentIndex);
                    if (!model || !model.pluginName) {
                        return;
                    }
                ...
                function determineCurrentIndex() {
                    ...
                    for (var i = 0; i < configDialog.wallpaperConfigModel.count; ++i) {
                        ...
                        if (data.pluginName === "org.kde.image") {
                            imageIndex = i;
                        }
                    ...
                    return configDialog.wallpaperConfigModel.count > 0 ? 0 : -1;
                }
            }
