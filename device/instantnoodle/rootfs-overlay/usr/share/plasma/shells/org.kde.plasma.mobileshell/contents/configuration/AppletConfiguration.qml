...
//END model
    ...
    // Qt 6.11 can complete the explicit Binding object before ConfigView has
    // placed this item in a window, which crashes QQmlBind on arm64. Apply the
    // same two window properties only after the attached window becomes valid.
    readonly property var configWindow: root.Window.window

    function configureWindow() {
        if (!configWindow) {
            return;
        }
        configWindow.flags = Qt.FramelessWindowHint;
        configWindow.visibility = Window.Maximized;
    }

    onConfigWindowChanged: configureWindow()
    Component.onCompleted: configureWindow()
